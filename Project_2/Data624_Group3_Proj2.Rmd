...
For this predictive model project, we've been provided two data sets - a training data set we will use to build the predictive models and an evaluation data set which will be used to predict unknown pH values and be scored to assess our model performance. Note that this is different from what you may have heard referred to as a train-test split. We use the training "TO MODEL" dataset to both train and test our models before using the best model to predict unknown pH scores for the submission dataset.
...
```{r}
cor_bev_train <- cor(bev_train[,-1], use = "na.or.complete")
corrplot(cor_bev_train, order = 'hclust', type = 'lower')
```
...
bev_train <- bev_train[complete.cases(bev_train[,26]),]
#update NA Brand Code records to "U"
bev_train$`Brand Code`[is.na(bev_train$`Brand Code`)] <- 'U'
```
...
```{r}
bev_train_i <- as.data.frame(bev_train[, !names(bev_train) %in% c("Brand Code", "PH")])
```

```{r}
#https://www.r-bloggers.com/missing-value-treatment/
bev_train_impute <- knnImputation(bev_train_i, k = 10)
bev_train_impute$pH <- bev_train$PH
bev_train_impute$'Brand Code' <- bev_train$`Brand Code`
```
...
## Regression Trees and Rule-Based Models - Alain
