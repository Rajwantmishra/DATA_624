...
# Data processing
trainDummy <- dummyVars(PH ~ ., data = bev_model, fullRank = TRUE)
trainX <- as.data.table(predict(trainDummy, newdata = bev_model))
predX <- as.data.table(predict(trainDummy, newdata = bev_score))


# Find missing values
missingDat <- bev_model[, lapply(.SD, function(x) sum(is.na(x)))]
missingTgt <- missDat$PH
...
```
The performance of these models are very similar, but Random forest seems to do slightly better than Extreme Gradient Boosting. We can apply the model to the evaluation set. 
